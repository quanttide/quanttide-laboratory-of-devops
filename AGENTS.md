...
- **数据持久化**：SQLite
- **CLI 名称**：`qtcloud-devops code`（二进制 `qtcloud-devops`）
...
│   ├── commands/       # 原子命令 + history + export
│   └── main.rs         # CLI 入口
├── src-tauri/          # Tauri 桌面应用壳
├── web-ui/             # 前端仪表盘（HTML/CSS/JS）
├── docs/
│   └── user-guide.md   # 用户指南
