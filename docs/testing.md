...
```python
def test_missing_changelog_detected():
    """只有 tag + Release，缺 CHANGELOG，验证 audit 发现不一致"""
```

```python
def test_tag_is_not_auto_fixed():
    """只有 CHANGELOG + Release 缺 tag，验证 provider 标记搁置而非自动修复"""
```
...
### 场景 4：正式版 CHANGELOG 汇总

```python
def test_formal_release_changelog_from_prerelease():
    """正式版发布时，从预发布 CHANGELOG 汇总而非从 git log 增量"""
```

### 场景 5：AI 绕过全链路

```python
def test_ai_bypass_full_cycle():
    """模拟 AI 跳过 publish、直接创建 tag+Release，验证 provider 扫描发现并修复"""
```
