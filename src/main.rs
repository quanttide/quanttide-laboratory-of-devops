...
fn main() {
    ...
    // 支持子命令路由: cargo run -- release publish <version>
    ...
    // ── 8. release status（发布状态，对应 devops-release skill） ────
    println!("━━━ 8. release status ━━━");
    release::status(&repo_path);
    println!();

    // ── 9. release precheck（前置检查，对应 devops-release skill） ──
    println!("━━━ 9. release precheck ━━━");
